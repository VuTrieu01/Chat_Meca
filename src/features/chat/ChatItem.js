...
export default function ChatItem({ openChatItem, chatArray, accounts, currentUser, dbRef, group, messenger }) {
     ...
     const chatGroup =
          group &&
          Object.values(messenger !== undefined && messenger)
     ...
     const countReadUser = group && chatGroup.filter((val) => val.unReadUser.find((it) => it === currentUser.uid)).length;
     const handleClick = (accounts, group) => {
          if (accounts) {
               chatArray
                    .filter((it) => it.accountId === accounts.uid && it.accountFriendId.includes(currentUser.uid) && it.newText === true)
                    .map((upItem) => {
                         return update(child(dbRef, `Chat/${accounts.uid}/${currentUser.uid}/${upItem.uid}`), {
                              newText: false,
                         });
                    });
          ...
          if (group) {
               chatGroup
                    .filter((it) => it.groupId === group.uid)
                    .map((upItem) => {
               ...
               <div className="flex items-center">
                    ...
                    <div className="w-full flex items-center justify-between">
                         <div className="mr-4">
                              ...
                              <div className={`w-48 ${accounts && chatData.newText && chatData.accountId !== currentUser.uid ? " font-bold text-green-600" : ""} text-sm whitespace-nowrap overflow-hidden overflow-ellipsis`}>
                                   {accounts && chatData.chat && chatData.chat} {accounts && chatData.img && "Đã gửi một ảnh"}
                              </div>
                              {group && chatGroupData !== undefined && (
                                   <div className="w-48 text-sm whitespace-nowrap overflow-hidden overflow-ellipsis">
                                        {chatGroupData.chat && chatGroupData.chat} {chatGroupData.img && "Đã gửi một ảnh"}
                                   </div>
                              )}
                         </div>
                         {accounts && (
                              <div className="flex flex-col items-end">
                                   ...
                                   <div className={`${chatData.newText && chatData.accountId !== currentUser.uid ? "flex items-center justify-center bg-red-600 text-white font-bold rounded-full text-xs" : ""} h-5 w-5 p-2`}>{chatData.newText && chatData.accountId !== currentUser.uid ? (countNewChat < 6 ? countNewChat : "+5") : ""}</div>
                              </div>
                         )}
                         {group && chatGroupData !== undefined && (
                              <div className="flex flex-col items-end">
                                   <div className="text-gray-400 text-sm">{ConvertNumberToTime(chatGroupData.lastLoggedInTime)}</div>
                                   <div className={`${chatGroupData.unReadUser.filter((it) => it === currentUser.uid) && countReadUser > 0 ? "flex items-center justify-center bg-red-600 text-white font-bold rounded-full text-xs" : ""} h-5 w-5 p-2`}>{chatGroupData.unReadUser.filter((it) => it === currentUser.uid) && countReadUser > 0 ? (countReadUser !== undefined && countReadUser < 6 ? countReadUser : "+5") : ""}</div>
                              </div>
                         )}
                    </div>
               </div>
