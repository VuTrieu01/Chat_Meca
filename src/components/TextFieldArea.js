     ...
     return (
          <div className={`${props.sx}`}>
               <div className="relative h-10 w-full min-w-[200px]">
                    <textarea onChange={props.onChange} value={props.value} name={props.name} type={props.type} className={`peer h-12 w-full rounded-[7px] border-x border-b px-3 py-2.5 text-sm font-normal transition-all placeholder-shown:border resize-none focus:border-2 focus:border-green-600 focus:border-t-transparent focus:outline-0 ${props.error}`} placeholder=" " disabled={props.disabled} />
                    <label
